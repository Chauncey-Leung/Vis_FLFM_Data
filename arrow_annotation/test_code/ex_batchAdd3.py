...
view_path_input.setPlaceholderText("输入视角 .npz 文件路径")
...
def save_snapshot_and_view():
    ...
    try:
        viewer.screenshot(snapshot_path, canvas_only=True, scale=4)  # scale=4 提高清晰度
        np.savez(view_path,
...
# layout.addWidget(QPushButton("Load Vectors", clicked=lambda: load_vectors_from_file()))
...
layout.addWidget(QPushButton("保存图像+视角", clicked=save_snapshot_and_view))
# layout.addWidget(QPushButton("同步指定视角", clicked=restore_view_from_file))
layout.addWidget(view_path_input)
layout.addWidget(QPushButton("同步视角（来自上面路径）", clicked=restore_view_from_textbox))
