...
def change_default_path():
    ...
    new_path = QFileDialog.getExistingDirectory(None, "选择新的图像文件夹", default_path)
    if not new_path:
        return
    ...
    if not tif_files:
        show_info("新路径下未找到 TIFF 文件")
        return

    path_input.setText(os.path.splitext(tif_files[0])[0] + '.json')
    load_path_input.setText(os.path.splitext(tif_files[0])[0] + '.json')
    load_tif(tif_index)
    show_info(f"路径已更新为：{default_path}")
...
change_path_btn = QPushButton("选择图像文件夹", clicked=change_default_path)
